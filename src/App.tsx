...
export function App(){
  ...
  if(auth.initializingError&&!auth.authenticated){
    return <AuthenticationFailure
      message={auth.initializingError}
      retry={auth.retryInitialization}
    />;
  }

  if(!auth.authenticated){
    return <LoginPage busy={auth.busy} login={auth.login}/>;
  }
    ...
    <header className="application-header">
      ...
      <div className="session-summary">
        <span>
          ...
          <small>Tenant {auth.tenantId||'not supplied'}</small>
        </span>
        <button
...
function LoginPage({
  busy,
  login
}:{busy:boolean;login:()=>Promise<void>}){
  return <main className="authentication-page">
    <section className="authentication-card" aria-labelledby="login-title">
      <div>
        <p className="brand-kicker">HRMS</p>
        <h1 id="login-title">Payroll foundation</h1>
        <p>
          Sign in through the payroll identity provider to access
          tenant-scoped payroll data and permitted actions.
        </p>
      </div>
      <div className="authentication-action">
        <button disabled={busy} onClick={()=>void login()}>
          {busy?'Opening sign in…':'Sign in with Keycloak'}
        </button>
        <small>
          Access tokens remain in browser memory and are not stored locally.
        </small>
      </div>
    </section>
  </main>;
...
function AuthenticationFailure({
  ...
  return <main className="authentication-page">
    <section className="authentication-card" aria-labelledby="auth-error-title">
      <div>
        <p className="brand-kicker">HRMS</p>
        <h1 id="auth-error-title">Authentication unavailable</h1>
        <p className="error" role="alert">{message}</p>
      </div>
      <div className="authentication-action">
        <button onClick={retry}>Retry authentication</button>
        <small>Confirm that the local Keycloak service is running.</small>
      </div>
    </section>
  </main>;
}
...
function NoAccessPage(){
  return <section className="card" aria-labelledby="no-access-title">
    <h2 id="no-access-title">No payroll access assigned</h2>
    <p role="alert">
      Your authenticated account has no supported read permission.
      Contact a payroll administrator.
    </p>
  </section>;
}
